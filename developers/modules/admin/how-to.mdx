...
While you cannot submit proposals directly, you can query the admin module's state:

```bash
# Query module parameters (if available)
neutrond query adminmodule params

# Query proposal by ID (if the query exists)
neutrond query adminmodule proposal <proposal-id>
```

<Note>
The specific query commands available depend on the module's implementation. Some queries may not be available or may have different names.
</Note>
