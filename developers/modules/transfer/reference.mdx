...
- **For contracts**: Fee validation is required if feerefunder `params.FeeEnabled` is set to true (see with `neutrond q feerefunder params`)
- **For non-contracts**: Fee validation is bypassedAt this point, for both contracts and non-contracts IBC fees are disabled

### 5. Automatic Fee Operations

Right now, fees are enabled.
If fees are globally disabled (see Params of feerefunder for that: `neutrond q feerefunder params`), contract won't be charged for fees.
