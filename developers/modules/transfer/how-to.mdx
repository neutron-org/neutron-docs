...
Right now, fees are disabled. That means you can submit no fees. Even if you specify fees in MsgTransfer, contract won't be charged.

If fees are globally enabled (see Params of feerefunder for that: `neutrond q feerefunder params`), the module automatically:
...
2. **If fees are enabled: Distributes Fees**: Calls appropriate fee distribution functions
