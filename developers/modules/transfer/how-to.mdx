...
Right now, fees are enabled (see `neutrond q feerefunder params`).
...
If fees are globally disabled (see Params of feerefunder for that: ), the module won't charge contract even if specified.
