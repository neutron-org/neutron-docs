...
The Dex module maintains several types of state to track pools, reserves, limit orders, and user positions.

## State Objects

The following objects are stored in the module's KVStore:
...
2. The tranche is moved from active to inactive storage
3. User positions are updated to reflect the cancellation

### Swap Execution

During a swap:

1. The swap algorithm iterates through ticks in price order
2. Pool reserves are updated as liquidity is consumed
3. Limit order tranches are filled, updating their reserves
4. User positions in filled tranches are updated
5. Trading fees are distributed to liquidity providers

## Parameters

The Dex module parameters control various aspects of the module's behavior:

```protobuf
message Params {
  // Trading fees in basis points (1/10,000)
  repeated uint64 fee_tiers = 1;
  
  // Tick spacing for each fee tier
  repeated uint64 tick_spacings = 2;
  
  // Maximum number of hops allowed in multi-hop swaps
  uint64 max_hops = 3;
  
  // Maximum number of ticks to iterate during a swap
  uint64 max_tick_iterations = 4;
}
``` 