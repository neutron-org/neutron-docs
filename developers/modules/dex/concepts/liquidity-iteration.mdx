...
The liquidity is distributed across different tick indexes:
- Tick 19640: 10 ATOM (Limit Order) at 7 USDC/ATOM
- Tick 20795: 10 ATOM (Pool) at 8 USDC/ATOM  
- Tick 21973: 10 ATOM (Pool) at 9 USDC/ATOM
- Tick 23027: 10 ATOM (Pool) at 10 USDC/ATOM
- Tick -19640: 10 USDC (Limit Order) at 0.14 ATOM/USDC
</Info>
...
<Info>
For the clarity of this example we are assuming that the `PoolReserves` have a fee of zero. In a real world scenario almost all `PoolReserves` will have a fee. In a scenario with fees `TokenIn` will be added to $Tick * -1 + (2\cdot fee$). Learn more about fees in the [PoolReserves](/developers/modules/dex/concepts/tick-liquidity#pool-reserves) section.
</Info>

### Example liquidity Placement Post Swap

<Info>
**Post-Swap Liquidity State**

After Alice's swap of 100 USDC:
- Tick 19640: 0 ATOM (Limit Order depleted)
- Tick 20795: 6.41 ATOM remaining (Pool partially used)
- Tick 21973: 10 ATOM (Pool unused)
- Tick 23027: 10 ATOM (Pool unused)
- Tick -20795: 28.73 USDC added (from Alice's swap)
</Info>
...
Learn more about the order types and their unique behaviors in the [Order Types](/developers/modules/dex/messages#order-types) section.