...
<Warning>
**Contract-Specific Examples**: All rate limit management examples below are specific to particular contract implementations and may not work with other rate limiting contracts. The actual queries and execute messages depend on the specific contract deployed. Only the core module messages (`send_packet`, `recv_packet`, `undo_send`) are standardized by the Go module.
</Warning>

Once the contract is set up, rate limits are managed through contract messages. These examples use the Neutron CLI for interaction with a typical rate limiting contract implementation.
