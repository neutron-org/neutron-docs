...
Fee locking only happens if `fee_enabled` param is set to true.
If it's set to false, it turns off min fee validation and lock fees mechanism.
...
To prevent spam and ensure relayers are properly compensated, the module enforces minimum fees (only if `fee_enabled` param set to true):
