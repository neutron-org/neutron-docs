...
<Note>
**Limited Query Commands**: The Fee Refunder module only provides `params` and individual packet `fee-info` queries. There are no CLI commands for querying minimum fees or listing all fee information.
</Note>
