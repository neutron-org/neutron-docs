...
The `add_schedule` message contains:
...
Submit the proposal to the Main DAO using the prePropose contract address:

```bash
# Submit proposal to Main DAO prePropose contract
neutrond tx wasm execute <main-dao-prepropose-address> \
  '{"propose": {"msg": {"propose": {...}}}}' \
  --from=<your-key> \
  --chain-id=neutron-1 \
  --gas=auto \
...
Once submitted, the proposal goes through the standard DAO governance process:

1. **Proposal Period**: Community can review the proposal
2. **Voting Period**: DAO members vote on the proposal
...
3. **Monitor Events**: Watch for `EventExecuteSchedule` events in block logs
...
Monitor these events for schedule activity:

- **`EventAddSchedule`**: Schedule was created successfully
- **`EventRemoveSchedule`**: Schedule was removed
- **`EventExecuteSchedule`**: Schedule execution attempt (check `success` attribute)
