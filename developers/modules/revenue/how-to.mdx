...
```bash
neutron query revenue validator-stats <validator_address>
```

### Using REST

```
GET /neutron/revenue/validator_stats
```
...
```json
{
  "stats": {
    "validator_info": {
      ...
      "commited_blocks_in_period": 980,
      "commited_oracle_votes_in_period": 950,
...
```bash
neutron tx gov submit-proposal update-params proposal.json --from=<key_name> --chain-id=<chain-id>
```
