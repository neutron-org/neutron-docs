...
Parameter updates can only be executed through governance proposals using the `MsgUpdateParams` message:
...
<Note>
This message can only be executed through governance proposals, not directly by individual users via CLI commands.
</Note>
