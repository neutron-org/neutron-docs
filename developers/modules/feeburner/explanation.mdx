...
The Fee Burner module processes fees at the end of each block from a designated module account.
...
```mermaid
graph TD
    A[Designated Module Account] --> B[Fee Burner Module]
    B --> C{Token Type?}
    C -->|NTRN| D[Burn Tokens]
    C -->|Non-NTRN| E[Send to Treasury]
    D --> F[Record Total Burned]
```
