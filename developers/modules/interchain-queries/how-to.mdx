...
This document provides step-by-step instructions for using the Interchain Queries module in your applications, focusing on smart contract integration for production use cases.

<Info>
This guide focuses on CosmWasm smart contract integration, which is the recommended approach for most applications. While CLI commands are available for testing and simple use cases, smart contracts provide better result processing, automatic updates, and integration with application logic. For CLI usage, see the [Client documentation](/developers/modules/interchain-queries/client).
</Info>
