---
title: "How To"
description: "Step-by-step guides for performing basic operations with the module"
---

<Info>
**Working Example Contract**: For a complete, working implementation, see the [neutron_interchain_queries example contract](https://github.com/neutron-org/neutron-sdk/tree/main/contracts/neutron_interchain_queries) in the neutron-sdk repository. Many developers find it easier to learn from working code.
</Info>
...
- [What's the role of IBC connections in Interchain Queries and how to choose one?](/developers/modules/interchain-queries/explanation#whats-the-role-of-ibc-connections-in-interchain-queries-and-how-to-choose-one)
...
**Might be interesting:**
- [How do TX Interchain Queries work?](/developers/modules/interchain-queries/explanation#how-do-tx-interchain-queries-work)
...
If no predefined helper function meets your needs, refer to the [How to register a KV Interchain Query with custom keys](/developers/modules/interchain-queries/how-to#how-to-register-and-handle-a-kv-interchain-query-with-custom-keys) section.
...
**Might be interesting:**
- [What are entry points and sudo calls?](/developers/modules/interchain-queries/explanation#what-are-entry-points-and-sudo-calls)
- [Limited gas for sudo calls](/developers/modules/interchain-queries/explanation#limited-gas-for-sudo-calls)
- [What happens if a sudo callback to a smart contract owning an Interchain Query fails?](/developers/modules/interchain-queries/explanation#what-happens-if-a-sudo-callback-to-a-smart-contract-owning-an-interchain-query-fails)
...
The [How to find out what transaction filter to use](/developers/modules/interchain-queries/how-to#how-to-find-out-what-transaction-filter-to-use) section provides detailed instructions for crafting a transaction filter.
...
- Performing [contract-side verification of submitted TX Interchain Query results](/developers/modules/interchain-queries/explanation#why-is-it-mandatory-to-do-contracts-side-verification-of-submitted-tx-interchain-query-results).
...
**Might be interesting:**
- [Why is it mandatory to do contract's side verification of submitted TX Interchain Query results?](/developers/modules/interchain-queries/explanation#why-is-it-mandatory-to-do-contracts-side-verification-of-submitted-tx-interchain-query-results)
- [What are entry points and sudo calls?](/developers/modules/interchain-queries/explanation#what-are-entry-points-and-sudo-calls)
- [Limited gas for sudo calls](/developers/modules/interchain-queries/explanation#limited-gas-for-sudo-calls)
- [What happens if a sudo callback to a smart contract owning an Interchain Query fails?](/developers/modules/interchain-queries/explanation#what-happens-if-a-sudo-callback-to-a-smart-contract-owning-an-interchain-query-fails) 