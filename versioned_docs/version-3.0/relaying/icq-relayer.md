...
All registered Interchain Queries and their parameters are stored in the eponymous module and available by its [query interface](/3.0/neutron/modules/interchain-queries/client#queries). The Relayer utilises the module's interface in order to initialise the performing list of queries. This is how the Relayer maintains the list of queries to be executed:
...
- during the rest of the run, the Relayer listens to the ICQ module's `query_update` and `query_removed` [events](/3.0/neutron/modules/interchain-queries/events) and modifies the queries list and parameters correspondingly.
...
Before running the Relayer application for production purposes, you need to [create a wallet for the Relayer](#setting-up-relayer-wallet), and set up the [configuration](#configuration).
