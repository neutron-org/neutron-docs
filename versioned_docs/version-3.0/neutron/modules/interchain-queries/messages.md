...
* `tx` - query to search for transactions on remote chain. `transactions_filter` describes a filter by which the [ICQ relayer](/relaying/icq-relayer) will perform the transactions search. [The transaction filter is described in more detail in the overview](/3.0/neutron/modules/interchain-queries/overview):
...
#### Events
Emits [`EventTypeNeutonMessage`](/3.0/neutron/modules/interchain-queries/events#eventtypeneutronmessage) with `action` equals `query_updated`.
...
#### Events
Emits [`EventTypeNeutonMessage`](/3.0/neutron/modules/interchain-queries/events#eventtypeneutronmessage) with `action` equals `query_removed`.
