...
```bash
git clone -b v9.0.3 https://github.com/cosmos/gaia.git
cd gaia
# for some reason, gaia v9.0.3 checks if you have go v1.18 installed,
# not newer, not older, however, it builds fine with go v1.20, so, before
# runnine `make install`, you will have to edit gaia's Makefile on line 106
# like this:
...
Gaia chain's RPC is exposed at `0.0.0.0:16657`, usable mnemonic: `banner spread envelope side kite person disagree path silver will brother under couch edit food venture squirrel civil budget number acquire point work mass`.

Neutron chain's RPC is exposed at `0.0.0.0:26657`, usable mnemonic: `veteran try aware erosion drink dance decade comic dawn museum release episode original list ability owner size tuition surface ceiling depth seminar capable only`.
