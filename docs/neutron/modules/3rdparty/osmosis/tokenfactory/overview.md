...
Note that since `TrackBeforeSend` hook can also be triggered upon module to module send (which is not gas metered), we internally gas meter `TrackBeforeSend` with a gas limit a limit defined by the module's `track_before_send_gas_limit` param.
