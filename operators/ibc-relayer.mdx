...
<Info>
Neutron introduces smart-contract level callbacks for IBC packets. From an IBC relayer's perspective, this means that custom application logic can be executed when a packet is submitted to Neutron, which can potentially drain the relayer's funds. This naturally brings us to a situation in which protocols would prefer to set up their own relayers and restrict the channels they are willing to relay for.
</Info>

For example, you can do this by adding a `chains.packet_filter` config:
...
<Note>
Please refer to the [official Hermes documentation](https://hermes.informal.systems/) if you want more deep understanding of config.
</Note>
...
<Info>
It is strongly recommended to run the Relayer as a daemon service to ensure automatic restarts and continuous operation.
</Info> 